# processing_reviewer.py - Add this to your project
import json
...
class ProcessingReviewer:
    ...
    def _assess_result_quality(self, result: Dict, item_num: int) -> float:
        ...
        if issues:
            # Add to global quality issues (you might want to track this differently)
            pass
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        success_rate = analysis['successful'] / analysis['total_processed'] if analysis['total_processed'] > 0 else 0
        ...
        if analysis['web_search_used'] > 0:
            web_search_failure_rate = analysis['web_search_failed'] / analysis['web_search_used']
        ...
        if stats.get('average_quality_score', 0) < 0.7:
            recommendations.append("Low average quality score. Review failed items and consider manual adjustments.")
        
        if stats.get('low_quality_count', 0) > stats.get('high_quality_count', 0):
...
• Successful: {analysis['successful']} ({analysis['successful']/analysis['total_processed']:.1%})
• Failed: {analysis['failed']} ({analysis['failed']/analysis['total_processed']:.1%})
...
• Web Search Success Rate: {(analysis['web_search_used'] - analysis['web_search_failed'])/analysis['web_search_used']:.1%} (if used)
...
# Usage example for your app.py
def add_processing_review_to_app(results):
    """Add this to your Flask app for processing review"""
    
    # After processing all titles, add this:
    ...
    # Create filtered high-quality results
    high_quality_results = reviewer.create_filtered_results(results, min_quality=0.6)
    
    # Export detailed CSV with quality metrics
    reviewer.export_detailed_csv(results, 'detailed_results_with_quality.csv')
    
    return quality_report, high_quality_results